...
const runWranglerJson = (args: string[]) => {
  ...
  return JSON.parse(stdout) as Array<{ results: MockQuestionRow[] }>;
};
...
const buildSql = (rows: MockQuestionRow[]) => {
  const lines: string[] = [];
...
const main = () => {
  const result = runWranglerJson([
    ...
    "DB",
    "--command",
    "SELECT q.id, q.bank_id, q.type, q.difficulty, qb.subject, qb.topic, qb.grade FROM questions q JOIN question_banks qb ON qb.id = q.bank_id WHERE q.id LIKE '%mock-%' ORDER BY q.id",
  ]);

  const rows = result[0]?.results ?? [];
  if (rows.length === 0) {
    console.log("No existing mock questions found.");
    return;
  }

  const sqlPath = join(tmpdir(), `pinequest-refresh-mock-${Date.now()}.sql`);
  ...
  console.log(`Refreshed ${rows.length} mock questions in ${targetEnv}.`);
};
